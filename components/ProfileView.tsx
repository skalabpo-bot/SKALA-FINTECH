    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in pb-10">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div><label className="text-xs font-bold text-slate-500 uppercase">Zona</label><select name="zoneId" value={formData.zoneId} onChange={handleChange} className="w-full p-2 border rounded bg-white text-slate-900"><option value="">Seleccione</option>{zones.map(z=><option key={z.id} value={z.id}>{z.name}</option>)}</select></div>
