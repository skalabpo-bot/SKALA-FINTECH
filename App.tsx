...
const App = () => {
           ...
           {authView === 'LOGIN' ? (
               ...
               <form onSubmit={handleRegister} className="space-y-6 max-h-[70vh] overflow-y-auto pr-4 custom-scrollbar">
                  ...
                  <div className="space-y-4">
                      ...
                      <select value={regData.zonaId} onChange={e=>setRegData({...regData, zonaId: e.target.value})} onFocus={async () => { const z = await MockService.getZones(); setRegZones(z); }} className="w-full px-6 py-4 bg-white border-2 border-slate-100 rounded-2xl font-bold outline-none focus:border-primary">
  ...
  return (
    <Layout currentUser={currentUser} onLogout={() => { cleanupAllSubscriptions(); setRadicarGestorId(''); setCurrentUser(null); setCurrentView('dashboard'); setEmail(''); setPassword(''); supabase.auth.signOut(); }} currentView={currentView} onChangeView={setCurrentView}>
      ...
      {currentView === 'academia' && MockService.hasPermission(currentUser, 'VIEW_ACADEMIA') && <AcademiaView currentUser={currentUser} />}
